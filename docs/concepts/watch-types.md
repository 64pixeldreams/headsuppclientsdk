...
The JSON examples below are the `watch_type` and `config` (plus optional `cooldown_seconds` / `recovery`) passed to `createWatch`.

Setup flow: [getting-started.md](../getting-started.md). Method reference: [client-reference.md](../client-reference.md). Scenario picker: [use-cases.md](use-cases.md).
